...
| `lock-not-released` | The apply finished — the tree was written and, if there was a previous tree, swept — but `.rdk/apply.lock` could not be removed afterward: the cause above names what's blocking it, most often a permission change under `.rdk/` since the apply started. | The generated tree is correct. Read the cause, clear whatever is blocking `.rdk/apply.lock`, then re-run. |
