...
**Tech Stack:** recent Go (`go.mod` tracks the current toolchain; rdk deliberately targets recent Go for itself and the projects it manages, and does not support trailing/EOL versions — see DD-17), `spf13/cobra` (CLI), `github.com/goccy/go-yaml` (parser, DD-12), stdlib `testing` + `google/go-cmp` (tests), `encoding/json` (deterministic `.tf.json` — Go sorts map keys).
