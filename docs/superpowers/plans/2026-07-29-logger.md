...
**Created:**
- `internal/diag/diag.go` — `Severity`, `Field`, `Diagnostic`, `Line()`
...
func TestNewSetsErrorSeverity(t *testing.T) {
	err := New(Diagnostic{Code: "unknown-kind", Summary: "x"})
...
// Fields exist for JSONL. In text they are already inside the summary, so
// printing them would say everything twice.
func TestFieldsAreNotPrintedInText(t *testing.T) {
...
func TestJSONLResultCarriesFields(t *testing.T) {
	...
	l.Result(diag.Diagnostic{
		...
		Fields:  []diag.Field{diag.Int("files", 4), diag.Str("dir", "rdk-managed")},
	})
...
func TestJSONLIsNeverColoured(t *testing.T) {
	...
	l := New(Options{Format: FormatJSONL, Level: slog.LevelDebug, Color: ColorAlways,
		Stdout: &out, Stderr: &errBuf, Env: noEnv})
	l.Warn(diag.Diagnostic{Code: diag.CodeArtifact, File: "a.yaml.bak", Summary: "leftover"})
...
func New(o Options) *Logger {
	...
	return &Logger{
		out: slog.New(newHandler(o, o.Stdout)),
		err: slog.New(newHandler(o, o.Stderr)),
	}
}

func newHandler(o Options, w io.Writer) slog.Handler {
	if o.Format == FormatJSONL {
		return slog.NewJSONHandler(w, &slog.HandlerOptions{Level: o.Level, ReplaceAttr: dropTime})
	}
	return &textHandler{w: w, level: o.Level, color: useColor(o.Color, w, o.Env), mu: &sync.Mutex{}}
}
...
func emit(l *slog.Logger, level slog.Level, d diag.Diagnostic, cause error) {
	...
	attrs := make([]slog.Attr, 0, 5+len(d.Fields))
	if d.Code != "" {
...
func TestEditorArtifactsWarn(t *testing.T) {
	for _, name := range []string{"assets.yaml.orig", "assets.yaml.rej", "assets.yaml.bak", "assets.yaml~"} {
		...
		if warnings[0].Code != diag.CodeArtifact {
			t.Errorf("%s: code = %q, want %q", name, warnings[0].Code, diag.CodeArtifact)
		}
	}
}
...
func (r Result) Diagnostic() diag.Diagnostic {
	return diag.Diagnostic{
		...
		Fields:   []diag.Field{diag.Int("files", r.FilesWritten), diag.Str("dir", ManagedDir)},
	}
}
```
...
Update the imports: drop `"fmt"` if it is now unused, add `"strings"` and `"github.com/jarrod-lowe/rdk/internal/diag"`.
	...
	return &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			a.log.Result(diag.Diagnostic{
				...
				Fields:   []diag.Field{diag.Str("version", version.Version)},
			})
			return nil
		},
	}
